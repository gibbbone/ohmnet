...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True,
    ...
    if learn_vectors:
        ...
        # uncomment the following line for hierarchical regularization
        if model.current_iteration > 0:
            # maximization vs. minimization
            l1 += alpha * (parent_vectors[context_index] - context_vectors[context_index])
...
class Word2Vec(utils.SaveLoad):
    ...
    def reset_weights(self):
        ...
        logger.info("resetting parent weights")
        self.syn_pi0 = zeros((len(self.vocab), self.vector_size), dtype=REAL)
    ...
    def load_parent_word2vec_format(self, fname, encoding='utf8', unicode_errors='strict'):
        """
        Loading weights from the internal nodes.
        """
        parent_count = 0
        logger.info("loading projection weights from %s" % (fname))
        with utils.smart_open(fname) as fin:
            ...
            for line_no, line in enumerate(fin):
                ...
                if word in self.vocab:
                    parent_count += 1
                    self.syn_pi0[self.vocab[word].index] = weights
        logger.info("loaded %d vectors into %s matrix from %s" % (parent_count, self.syn_pi0.shape, fname))
